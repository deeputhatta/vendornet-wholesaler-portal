...
export default function Login() {
  ...
  const verifyOTP = async () => {
    ...
    try {
      ...
      if (!user.role?.includes('wholesaler')) {
        setError('Access denied — wholesalers only');
        return;
      }
      login({ token: res.data.token, refresh_token: res.data.refresh_token }, user);
      navigate('/');
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100vw', background: '#000', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#1C1C1E', border: '1px solid #2C2C2E', borderRadius: 20, padding: 40, width: 380 }}>
        ...
        {step === 'mobile' ? (
          <>
            ...
            <div style={{ display: 'flex', alignItems: 'center', background: '#2C2C2E', border: '1px solid #3A3A3C', borderRadius: 10, marginBottom: 16, paddingLeft: 14, overflow: 'hidden' }}>
              <span style={{ color: '#8E8E93', fontSize: 14, marginRight: 8 }}>+91</span>
              <input
          ...
          <>
            ...
            <button onClick={() => { setStep('mobile'); setOtp(''); setError(''); }} style={{ width: '100%', background: 'transparent', border: 'none', color: '#0A84FF', fontSize: 13, cursor: 'pointer' }}>
              Change number
            </button>
          </>
        )}
